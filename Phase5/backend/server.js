...
let products = [
  ...
  { id: 10, name: "Drone", description: "Quadcopter drone with HD camera and 30 minutes flight time.", price: 8999, image: "Drone.jpeg" },
...
// ================= ROUTES ================= //
// Welcome route
app.get("/", (req, res) => res.send("<h1>Welcome to Classic Shop Backend 🚀</h1>"));
...
app.post("/orders", (req, res) => {
  ...
  const newOrder = { id: Date.now(), items, customer, createdAt: new Date().toLocaleString() };
...
app.get("/orders", (req, res) => {
  if (orders.length === 0) return res.send("<h2>No orders found yet 🚀</h2>");
  let html = `<h2>🧾 Orders List</h2><ul style="list-style:none;padding:0">`;
  orders.forEach(order => {
    html += `<li style="margin:15px 0;padding:10px;border:1px solid #ccc;border-radius:10px;">
...
// Show orders as JSON
app.get("/orders/json", (req, res) => res.json(orders));
